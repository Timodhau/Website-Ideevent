...
if(isset($_SESSION['client_id'])){
	...
	if($donnee=$requete->fetch()){
		$base->exec('UPDATE participer_suivre_noter SET psn_noter=4 WHERE psn_client='.$_SESSION['client_id'].' AND psn_event='.$_SESSION['event'].'');
		header('Location: ../SiteEvenement.php?event='.$_SESSION['event'].'');}
	else{$base->exec('INSERT INTO participer_suivre_noter(psn_client, psn_event, psn_noter) VALUES('.$_SESSION['client_id'].','.$_SESSION['event'].',4)');
			header('Location: ../SiteEvenement.php?event='.$_SESSION['event'].'');};}
