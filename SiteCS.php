...
$requete=$base->query('SELECT p.psn_event, e.event_titre, e.event_datedebut, e.event_description FROM participer_suivre_noter p LEFT JOIN event e ON e.event_id=p.psn_event WHERE p.psn_client='.$idclient.' AND p.psn_suivre=1 ORDER BY event_datedebut ASC');
$nbentree=0;
        ...
        <!-- barre de recherche -->
    		...
    		<section>
				...
				<div id="accueil">
					<?php 
					include("Include/connect_base.php");
					$requete=$base->query("SELECT p.psn_event, e.event_titre, e.event_datedebut, e.event_description FROM participer_suivre_noter p LEFT JOIN event e ON e.event_id=p.psn_event WHERE p.psn_client=$idclient AND p.psn_suivre=1 ORDER BY event_datedebut LIMIT $dbe,4");
